...
const int ci_Ultrasonic_Ping2 = 7;   //input plug
const int ci_Ultrasonic_Data2 = 4;   //output plug

const int ci_Ultrasonic_Ping3 = 16;   //input plug
const int ci_Ultrasonic_Data3 = 19;   //output plug
...
void setup() {
  ...
  pinMode(ci_Ultrasonic_Ping2, OUTPUT);// Trig
  pinMode(ci_Ultrasonic_Data2, INPUT); // Echo
  
  pinMode(ci_Ultrasonic_Ping3, OUTPUT);
  pinMode(ci_Ultrasonic_Data3, INPUT);
...
double Ping2()
{
  digitalWrite(ci_Ultrasonic_Ping2, HIGH);
  delayMicroseconds(10); //The 10 microsecond pause where the pulse in "high"
  digitalWrite(ci_Ultrasonic_Ping2, LOW);
  //use command pulseIn to listen to Ultrasonic_Data pin to record the
  //time that it takes from when the Pin goes HIGH until it goes LOW 
  duration = pulseIn(ci_Ultrasonic_Data2, HIGH, 10000);
...
double Ping3()
{
  digitalWrite(ci_Ultrasonic_Ping3, HIGH);
  delayMicroseconds(10); //The 10 microsecond pause where the pulse in "high"
  digitalWrite(ci_Ultrasonic_Ping3, LOW);
  //use command pulseIn to listen to Ultrasonic_Data pin to record the
  //time that it takes from when the Pin goes HIGH until it goes LOW 
  duration2 = pulseIn(ci_Ultrasonic_Data3, HIGH, 10000);
  //Serial.print(duration/58);
  //Serial.print(" . ");
  return duration;
